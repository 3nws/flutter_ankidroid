...
class Ankidroid {
  ...
  static Future<Ankidroid> createAnkiIsolate() async {
    ...
    final isolate =
        await FlutterIsolate.spawn(_isolateFunction, rPort.sendPort);
    final ankiPort = await rPort.first;

    return Ankidroid._(isolate, ankiPort as SendPort);
  }
  ...
  static Future<void> _isolateFunction(SendPort sendPort) async {
    ...
    ankiPort.listen((event) async {
      event["sendPort"].send(
          await futureToResultMap(() async => await methodChannel.invokeMethod(
              event["functionName"] as String,
              event
                ..remove("functionName")
                ..remove("sendPort"))));
    });
  ...
  Future<Result<String>> test() async {
    ...
    return mapToResult(ret as Map<String, dynamic>);
  }
  ...
  Future<Result<int>> addNote(
      int modelId, int deckId, List<String> fields, List<String> tags) async {
    final rPort = ReceivePort();
    _ankiPort.send({
  ...
  Future<Result<int>> addNotes(int modelId, int deckId,
      List<List<String>> fieldsList, List<List<String>> tagsList) async {
    final rPort = ReceivePort();
    _ankiPort.send({
  ...
  Future<Result<String>> addMedia(
      Uint8List bytes, String preferredName, String mimeType) async {
    final rPort = ReceivePort();
    _ankiPort.send({
      "functionName": "addNotes",
      "sendPort": rPort.sendPort,
  ...
  Future<Result<List<dynamic>>> findDuplicateNotesWithKeys(
      int mid, List<String> keys) async {
    final rPort = ReceivePort();
    _ankiPort.send({
  ...
  Future<Result<Map<dynamic, dynamic>>> previewNewNote(
      int mid, List<String> flds) async {
    final rPort = ReceivePort();
    _ankiPort.send({
  ...
  Future<Result<int>> currentModelId() async {
    final rPort = ReceivePort();
    _ankiPort
        .send({"functionName": "currentModelId", "sendPort": rPort.sendPort});
  ...
  Future<Result<String>> selectedDeckName() async {
    final rPort = ReceivePort();
    _ankiPort
        .send({"functionName": "selectedDeckName", "sendPort": rPort.sendPort});
  ...
  Future<Result<int>> apiHostSpecVersion() async {
    final rPort = ReceivePort();
    _ankiPort.send(
        {"functionName": "apiHostSpecVersion", "sendPort": rPort.sendPort});
